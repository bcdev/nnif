 ...
 */
#define NNFT_VERSION_INFO    "Version 1.3"  
...
{		
	...
	for (nNumRecords = 0; !feof(istream); nNumRecords++)
	{
		for (i = 0; i < nNumInpUnits; i++) 
        {
			if (fscanf(istream, "%lf", &pdInpT[i]) != 1) 
            {
				if (i == 0)
					break;
				fprintf(stderr, "Error: file '%s' has an invalid format after %d records\n", pszIFile, nNumRecords);
				exit(-1);
			}
		}
		if (i == 0)
        ...
        {
			if (fscanf(istream, "%lf", &pdOutT[i]) != 1) 
            {
				fprintf(stderr, "Error: file '%s' has an invalid format after %d records\n", pszIFile, nNumRecords);
				exit(-1);
			}
